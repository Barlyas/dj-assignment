...
public:
    ...
    T& operator*() const {
        if (ptr == nullptr) {
            throw std::runtime_error("Attempting to derefernce null pointer PointerWrapper");
        }
        return *ptr;
    };
    ...
    T* operator->() const {
        if (ptr == nullptr) {
            throw std::runtime_error("Attempt to use operator->() on a null PointerWrapper");
        }
        return ptr;
    }
    ...
    T* get() const {
        if (ptr == nullptr) {
            throw std::runtime_error("Attempt to access a null pointer");
        }
        return ptr;
    }
